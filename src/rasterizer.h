...
private:
  ...
  // helper function to determine if point (testX, testY) is inside the triangle
  bool inside(float x0, float y0, float x1, float y1,
              float x2, float y2, float testX, float testY) {
      bool line1, line2, line3;
      
      line1 = -(testX - x0) * (y1 - y0) + (testY - y0) * (x1 - x0) >= 0;
      line2 = -(testX - x1) * (y2 - y1) + (testY - y1) * (x2 - x1) >= 0;
      line3 = -(testX - x2) * (y0 - y2) + (testY - y2) * (x0 - x2) >= 0;

      return (line1 && line2 && line3) || (!line1 && !line2 && !line3);
  }

  // helper function that computes baycentric coords (alpha, beta, epsilon), and returns true if (testX, testY) is inside triangle
  bool baycentricCoords(float x0, float y0, float x1, float y1,
      float x2, float y2, float testX, float testY, float &alpha, float &beta, float &epsilon) {
      
      alpha = (-(testX - x1) * (y2 - y1) + (testY - y1) * (x2 - x1)) / (-(x0 - x1) * (y2 - y1) + (y0 - y1) * (x2 - x1));
      beta = (-(testX - x2) * (y0 - y2) + (testY - y2) * (x0 - x2)) / (-(x1 - x2) * (y0 - y2) + (y1 - y2) * (x0 - x2));
      epsilon = 1.0 - alpha - beta;

      return (alpha >= 0.0 && beta >= 0.0 && epsilon >= 0.0) || (alpha < 0.0 && beta < 0.0 && epsilon < 0.0);
  }
